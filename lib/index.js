    ...
    keys.forEach(function (key) {
        ...
        if(task) {
            ...
            targets.forEach(function (target) {
                ...
                tgt.src         = util._.isArray(tgt) ? tgt : tgt.src;
                tgt.dest        = tgt.dest;

                // inherit from parent

                tgt.options     = util._.merge({}, opts, tgt.options);  // merge task default options with targets
                tgt.deps        = util._.merge([], deps, tgt.options);  // merge task default deps with targets
                tgt.description = cfg.description || tgt.description || util.format('Run the "%s" task using target "%s"', name, target);
                tgt.help        = util._.isBoolean(tgt.help) ? tgt.help : true; // should taks be displayed in help

                // create task

                args = [util.format('%s:%s', name, target), tgt.help && tgt.description, tgt.deps, function () {
