    ...
    keys.forEach(function (key) {
        ...
        if(task) {
            ...
            targets.forEach(function (target) {
                ...
                file  = { src : tgt.src, dest: tgt.dest };
                files = tgt.files || file.src && [file] || [];
                files = _.isPlainObject(files) ? _.reduce(files, function(result, value, key) {

                    result.push({ src : value, dest: key });
                    return result;
                ...
                args = [tname, tgt.deps, function () {
                    ...
                    scope.file     = file;
                    scope.files    = files;
                    scope.options  = util.options.bind(tgt.options);

                    return task.apply(scope, [gulp].concat(args));
